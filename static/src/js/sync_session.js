odoo.define('pos_retail.synchronization', function (require) {
        ...
        button_click: function () {
            ...
            if (orders.length) {
                return rpc.query({
                    ...
                    context: {}
                }).then(function () {}).fail(function (type, error) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        syncing_sessions: function (message) {
            ...
            if (this.config.sync_action_automatic) {
            ...
            if (action == 'sent_order') {
                // console.log("Se envio una orden");
                // let order = message['data'];
                // this.db.save_data_sync_sent_order(order);
                // this.db.save_shipment_order(order);
        ...
        sync_change_pricelist: function (vals) {
            var order = this.get_order_by_uid(vals['uid'])
            var pricelist = _.findWhere(this.pricelists, {
                id: vals['pricelist_id']
            });
            if (!order || !pricelist) {
